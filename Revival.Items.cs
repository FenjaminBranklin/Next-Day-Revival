...
namespace NextDayRevival
{
    ...
    public static class ResourceHook
    {
        ...
        public static bool Prefix(string path, ref UnityEngine.Object __result)
        {
            ...
            if (path.Equals(RevivalPlugin.StingerScopePath, StringComparison.OrdinalIgnoreCase))
            {
                Texture2D scope = Assets.Texture("stinger_scope.png", false, false);
                if (scope == null) return true;
                Log(path, "Zielfernrohr Stinger");
                __result = scope;
                return false;
            }
    ...
    public static class Registry
    {
        ...
        // 2067 is the parachute (Revival.Parachute.cs). It is in the shop and
        // the mine and the repair items are not, for a reason that is about the
        // feature and not about the price list: a parachute is spent on the jump
        // that uses it, so a player who cannot buy one has the helicopter's bail
        // out exactly once, and then never again.
        // 2068 is the Stinger's reload round. It has to be buyable for the
        // same reason the parachute is: the launcher (45000) is a gripstock
        // that keeps working, so without a source of tubes it would still be
        // the one-shot weapon it stopped being. Priced at 20000 - well under
        // half the launcher, so the second shot is affordable and the third
        // one hurts.
